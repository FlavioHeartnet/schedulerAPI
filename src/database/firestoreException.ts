export function localizeErrorsMap(e) {
    if (typeof e === 'object' && typeof e.code === 'string' && e.code in firestoreErrors)
        e.message = (firestoreErrors as any)[e.code];
...
const firestoreErrors = {
    'canelled': "A operação foi cancelada",
    'unknown': "Erro desconhecido ou um erro de um domínio de erro diferente.",
    'invalid-argument': "argumento inválido': O cliente especificou um argumento inválido. Observe que isso difere de 'pré-condição com falha",
    'deadline-exceeded': "O prazo expirou antes que a operação pudesse ser concluída. Para operações que alteram o estado do sistema, esse erro pode ser retornado mesmo que a operação tenha sido concluída com êxito",
    'not-found': "Documento não encontrado",
    'already-exists': "Documento já existe",
    'permission-denied': " O Caller não tem permissão para executar a operação especificada.",
    ...
    'aborted': "A operação foi abortada, normalmente devido a um problema de simultaneidade.",
